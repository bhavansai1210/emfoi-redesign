...
const caseStudyDrafts = [
  { number: "01", sector: "Government", title: "Project title", context: "Client / agency", scope: "Scope of work", delivery: "Delivery approach", outcome: "Approved outcome" },
  { number: "02", sector: "Healthcare", title: "Project title", context: "Care organization", scope: "Scope of work", delivery: "Delivery approach", outcome: "Approved outcome" },
  { number: "03", sector: "Enterprise", title: "Project title", context: "Organization", scope: "Scope of work", delivery: "Delivery approach", outcome: "Approved outcome" },
];
...
export default function Home() {
  ...
  return (
    <><PageSEO title="EMFOI | Software, AI & Staffing for Government and Healthcare" description="EMFOI delivers custom software, AI development and governance, IT consulting, and healthcare staffing for government, healthcare, and complex enterprise organizations." canonicalPath="/" /><PageLayout>
      ...
      <section className="case-study-ready-section"><MotionReveal className="site-width"><div className="case-study-ready-intro"><div><p className="eyebrow"><span />Case studies</p><h2>Project records, ready for your real work.</h2></div><div className="case-study-release-note"><span>Editable project collection</span><p>Each highlighted field below is ready to be replaced with client-approved project detail, delivery evidence, and outcomes.</p></div></div><motion.div className="case-study-ready-grid case-study-draft-grid" variants={scrollGroup} initial={reducedMotion ? false : "hidden"} whileInView={reducedMotion ? undefined : "visible"} viewport={{ once: true, amount: 0.22 }}>{caseStudyDrafts.map((study) => <motion.article className="case-study-ready-card case-study-draft-card" key={study.number} variants={scrollItem}><div className="case-study-card-head"><span>{study.number} / CASE STUDY</span><span>Editable</span></div><div className="case-study-card-copy"><p>{study.sector} / Portfolio</p><h3>{study.title}</h3><span className="case-study-context">{study.context}</span></div><dl className="case-study-draft-fields"><div><dt>Scope</dt><dd>{study.scope}</dd></div><div><dt>Delivery</dt><dd>{study.delivery}</dd></div><div><dt>Outcome</dt><dd>{study.outcome}</dd></div></dl><div className="case-study-card-rail"><span>Replace with approved project evidence</span><ArrowRight size={14} /></div></motion.article>)}</motion.div><p className="case-study-ready-note">To publish a final record, replace each field with client-approved content. <Link href="/contact">Plan the project conversation <ArrowRight size={14} /></Link></p></MotionReveal></section>
