...
const caseStudySlots = [
  { number: "01", label: "Public-sector delivery", title: "Mission systems & program support", text: "A dedicated case brief will document approved scope, delivery approach, and publishable outcomes for government work." },
  { number: "02", label: "Healthcare operations", title: "Usable systems around care", text: "A verified engagement profile will be added once the organization, delivery context, and results are cleared for publication." },
  { number: "03", label: "Enterprise delivery", title: "Specialized capability in context", text: "A concise project record will be published with company-approved details about the work, role, and operational outcome." },
];
...
export default function Home() {
  ...
  return (
    <><PageSEO title="EMFOI | Software, AI & Staffing for Government and Healthcare" description="EMFOI delivers custom software, AI development and governance, IT consulting, and healthcare staffing for government, healthcare, and complex enterprise organizations." canonicalPath="/" /><PageLayout>
      ...
      <section className="case-study-ready-section"><MotionReveal className="site-width"><div className="case-study-ready-intro"><div><p className="eyebrow"><span />Case studies</p><h2>Work records, ready for approved detail.</h2></div><div className="case-study-release-note"><span>Publication protocol</span><p>EMFOI will publish named project facts, scope, and outcomes only when client-approved.</p></div></div><motion.div className="case-study-ready-grid" variants={scrollGroup} initial={reducedMotion ? false : "hidden"} whileInView={reducedMotion ? undefined : "visible"} viewport={{ once: true, amount: 0.22 }}>{caseStudySlots.map((study) => <motion.article className="case-study-ready-card" key={study.number} variants={scrollItem}><div className="case-study-card-head"><span>{study.number} / CASE FILE</span><span>Content pending</span></div><div className="case-study-card-copy"><p>{study.label}</p><h3>{study.title}</h3><p>{study.text}</p></div><div className="case-study-card-rail"><span>Scope</span><i /><span>Delivery</span><i /><span>Outcome</span></div></motion.article>)}</motion.div><p className="case-study-ready-note">Have a project you can approve for publication? <Link href="/contact">Start a conversation with EMFOI <ArrowRight size={14} /></Link></p></MotionReveal></section>
