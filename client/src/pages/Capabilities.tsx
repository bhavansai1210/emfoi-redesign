    ...
    <><PageSEO title="EMFOI Capabilities | Software, AI & Staffing for Government and Healthcare" description="Explore EMFOI capabilities in custom software and UX, AI development and governance, IT consulting, staffing, cyber security, managed services, cloud, network connectivity, ERP, and IT modernization." canonicalPath="/capabilities" breadcrumbs={[{ name: "Capabilities", path: "/capabilities" }]} /><PageLayout>
      ...
      <section className="paper-section capabilities-index-section" ref={dossierRef}>
        <div className="site-width capability-list">
          ...
          <div className="capability-index-group" data-aos="fade-up"><div className="capability-index-group-heading editorial-grid"><div><p className="eyebrow"><span />Core practices</p><h2>Start with the primary delivery need.</h2></div><p>These four practices describe the center of EMFOI’s current software, AI, consulting, and staffing work.</p></div><div className="capability-card-grid">{practiceCards.map((card) => <Link key={card.slug} href={`/capabilities/${card.slug}`} className="capability-card"><span>{card.number}</span><div><h3>{card.title}</h3><p>{card.short}</p></div><ChevronRight size={21} /></Link>)}</div></div>
          <div className="capability-index-group" data-aos="fade-up"><div className="capability-index-group-heading editorial-grid"><div><p className="eyebrow"><span />Connected service lines</p><h2>Bring the surrounding systems into view.</h2></div><p>The broader EMFOI service taxonomy is presented here as focused modules for security, operations, infrastructure, enterprise systems, and modernization.</p></div><div className="capability-card-grid">{serviceCards.map((card) => <Link key={card.slug} href={`/capabilities/${card.slug}`} className="capability-card"><span>{card.number}</span><div><h3>{card.title}</h3><p>{card.short}</p></div><ChevronRight size={21} /></Link>)}</div></div>
        </div>
      </section>
      <section className="site-width capability-index-note"><p><Check size={18} />All EMFOI development and support programs follow established practices for HIPAA, Section 508, and NIST-aligned delivery where appropriate.</p></section>
...
function CapabilityDetail() {
  ...
  const slug = location.split("/").pop() as Slug;
  const data = capabilityData[slug];
  if (!data) return <CapabilityIndex />;

  return (
    <><PageSEO title={`${data.title} | EMFOI Capabilities`} description={data.intro} canonicalPath={`/capabilities/${slug}`} breadcrumbs={[{ name: "Capabilities", path: "/capabilities" }, { name: data.title, path: `/capabilities/${slug}` }]} /><PageLayout>
      <PageHero eyebrow={data.eyebrow} title={data.title} intro={data.intro}><Link href="/contact" className="button button-secondary">Discuss this capability <ArrowRight size={16} /></Link></PageHero>
      <section className="paper-section capability-detail-section"><div className="site-width capability-detail-grid editorial-grid" data-aos="fade-up"><div><p className="eyebrow"><span />Practice focus</p><h2>Support built for the program context.</h2><ul className="check-list">{data.focus.map((item) => <li key={item}><Check size={17} />{item}</li>)}</ul></div><figure><img src={data.image} alt={data.imageAlt} loading="lazy" /><figcaption>EMFOI / {data.title}</figcaption></figure></div></section>
      <section className="site-width capability-detail-copy editorial-grid" data-aos="fade-up"><div><p className="eyebrow"><span />Delivery detail</p><h2>High-stakes execution.</h2></div><p className="large-copy">{data.detail}</p></section>
      <section className="ink-cta-section"><div className="site-width ink-cta-grid"><div><p className="eyebrow eyebrow-light"><span />Next step</p><h2>Ready to explore a delivery conversation?</h2></div><p>Contact the EMFOI team to discuss how this capability can support your program, product, or staffing needs.</p><Link href="/contact" className="button button-light">Contact EMFOI <ArrowRight size={16} /></Link></div></section>
    </PageLayout></>
  );
}
...
export default function Capabilities() {
  const [location] = useLocation();
  return location === "/capabilities" ? <CapabilityIndex /> : <CapabilityDetail />;
}
