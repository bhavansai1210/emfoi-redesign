...
If the repository is cloned to a local machine, run `pnpm assets:sync -- --public` from the project root. The command downloads the 27 current managed-storage assets into `client/public/asset-backup/local/` for local development and writes `SOURCE_URLS.json` beside them. That local cache is intentionally ignored by Git and is not included in production checkpoints; production continues to use the existing `/manus-storage/` URLs. Running `pnpm assets:sync` without `--public` restores the same files to the external local cache at `/home/ubuntu/webdev-static-assets/emfoi-local-assets/`.
